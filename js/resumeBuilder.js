/*
  ...
  * avoid the use of non-polyfill ES6 features such as `const` and `let` as
    these will throw errors on older JS engines such as that used on the
    iPhone 4 running iOs 8.1;
  ...
  * prefer `$(obj).append(_window.Resume.markup.HTMLString_)` as opposed to
    `$(obj).append($(_window.Resume.markup.HTMLString_))` as it performs at least 5%
...
(function() {
  ...
  * @returns {string} New string
  */
  var _replaceData = function(content, template) {
  ...
  */
  ...
  window.Resume.bio = {
    ...
    welcomeMessage: "A web developer with over 10 years of professional" +
      " experience. Clients include small startups through to established" +
      " multi-nationals. Solutions incorporate simple websites to large web" +
      " applications. Specialties include JavaScript, CSS, HTML, Node.js," +
      " Dart, PHP, open-source and web performance.",
    ...
    display: function() {
      ...
      */
      ...
      /*
        ...
        Contact channels.
      */
      topContacts.append(window.Resume.printContactInfo());
  ...
  /*
    Additional content that extends the window.Resume.bio.contacts object.
    Intentionally defined in a separate content block for DRY purposes.
  */
  window.Resume.bio.contactsMeta = {
    location: {
      icon: "fa-map-marker",
    ...
    email: {
      icon: "fa-envelope",
      link: "mailto:" + window.Resume.bio.contacts.email,
      target: false
    },
    mobile: {
      icon: "fa-phone",
      link: "tel:" + window.Resume.bio.contacts.mobile,
      target: false
    },
    github: {
      icon: "fa-github",
      link: "//github.com/" + window.Resume.bio.contacts.github
    }
  };

  /* Education at a glance */
  window.Resume.education = {
    schools: [{
  ...
  window.Resume.work = {
    jobs: [{
      ...
      },
      {
        ...
        description: "Having previously worked for VYRE prior to my sabbatical" +
          " to Canada (see below), I resumed my employment with the company who" +
          " had been acquired by North Plains Ltd. As a Technical Consultant I" +
          " lead a team of developers, designers and testers to produce" +
          " solutions built on the company's bespoke Content Management" +
  ...
  window.Resume.footer = {
    display: function() {
      $('.footer .menu').append(window.Resume.printContactInfo());
    }
  };

  window.Resume.printContactInfo = (function _generateContactInfo(channels) {
    ...
    Object.keys(channels).forEach(function(channel) {
      ...
      meta = window.Resume.bio.contactsMeta[channel];
    ...
    /*
      Thanks to this closure, each window.Resume.printContactInfo() invocation
      will now receive a unique cloned instance of the original DOMFragment; not
      using a closure would result in a single DOMFragment, which would be moved
      to a new parent DOMElement with *each* function invocation (ie. the header
  ...
  })(window.Resume.bio.contacts);

  window.Resume.bio.display();
  window.Resume.work.display();
  window.Resume.projects.display();
  window.Resume.education.display();
  window.Resume.footer.display();
})();
